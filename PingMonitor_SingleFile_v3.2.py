...
def ping_once():
 ...
 p=subprocess.run(cmd,capture_output=True,text=True,startupinfo=startupinfo,creationflags=creationflags)
...
def worker():
 ...
 while running:
  ...
  time.sleep(float(load_var.get() or 1))
def start():
 ...
 if not running:
  ...
  load_entry.config(state="disabled")
  threading.Thread(target=worker,daemon=True).start()
def stop():
 global running
 running=False
 ip_entry.config(state="normal")
 load_entry.config(state="normal")
...
load_var=tk.StringVar(value="1")
load_entry=ttk.Entry(top,textvariable=load_var,width=5)
